...
class BiometricApp:
    def __init__(self, root):
        ...
        self.eye_label = tk.Label(self.images_frame, image=self.eye_photo, bg="#f0f4f7", text="Eye Tracking", compound="top", font=("Arial", 10, "bold"))
        self.mouse_label = tk.Label(self.images_frame, image=self.mouse_photo, bg="#f0f4f7", text="Mouse Tracking", compound="top", font=("Arial", 10, "bold"))
    ...
    def end_trial(self):
        ...
        if self.trial == 1:
            ...
            # Eye similarity using MSE as a rough proxy (you could normalize this too)
            mse_eye = np.mean((self.trial1_data[3:] - feature_vector[3:]) ** 2)
            eye_similarity = max(0, 100 - mse_eye / 5)  # You can fine-tune this scale
